...
describe('Post', () => {
  test('should render correctly post correctly when not in preview mode', () => {
    ...
    expect(screen.getByRole('heading', { level: 3 })).toHaveTextContent(mockedPost.title);

    expect(screen.getByText(mockedPost.body)).toBeInTheDocument();
    expect(screen.getByText(mockedPost.author.firstName, { exact: false })).toBeInTheDocument();
    expect(screen.getByText(mockedPost.author.lastName, { exact: false })).toBeInTheDocument();
  });
