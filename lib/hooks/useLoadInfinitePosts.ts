...
export const useLoadInfinitePosts = (props: PostsWithAuthorResponse) => {
  const { data, isLoading, error, fetchNextPage, isFetching } = useGetInfinitePostsInfiniteQuery({
    limit: props.limit,
    skip: props.skip
  });
  ...
  const loadMorePosts = entry?.isIntersecting && !isFetching && !isLoading && hasMore;
